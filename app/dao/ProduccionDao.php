...
class ProduccionDao
{
    ...
    {
        $insumos = array();
        $stmt = $this->conn->prepare("SELECT i.nom_ins AS insumo, DATE(p.fech_ini) AS fecha, SUM(i.stock)/1000 AS total_stock FROM produccion p INNER JOIN insumotoproduccion itp ON p.cod_procc = itp.cod_procc INNER JOIN insumo i ON itp.cod_ins = i.cod_ins WHERE p.fech_ini >= '2026-04-01' GROUP BY i.nom_ins, DATE(p.fech_ini) ORDER BY fecha DESC;");
        $stmt->execute();
        $result = $stmt->get_result();
        while ($row = $result->fetch_assoc()) {
            $insumo = new Insumo(
    ...
    public function getInsumosPreciosinProduccion(){
        $insumos = array();
        $stmt = $this->conn->prepare("SELECT i.nom_ins, sum(i.precio_tot) AS precio_total, DATE(p.fech_ini) AS fecha FROM produccion p INNER JOIN insumotoproduccion itp ON p.cod_procc = itp.cod_procc INNER JOIN insumo i ON i.cod_ins = itp.cod_ins WHERE i.uni_med = 'S/D' GROUP BY i.nom_ins, p.fech_ini ORDER BY p.fech_ini DESC");
        $stmt->execute();
        $result = $stmt->get_result();
        while ($row = $result->fetch_assoc()) {
            $insumo = new Insumo(
    ...
    {
        $productos = array();
        $stmt = $this->conn->prepare("SELECT pr.nom_prod as producto, pr.tam_prod as tamaño, p.cant_procc as cantidad, p.cant_extra as extra, DATE(p.fech_ini) as fecha from produccion p inner join productotoproducc ptop on p.cod_procc = ptop.cod_procc inner join producto pr on ptop.cod_prod = pr.cod_prod WHERE fech_ini >= '2026-04-01' order by p.fech_ini desc;");
        $stmt->execute();
        $result = $stmt->get_result();
        while ($row = $result->fetch_assoc()) {
            $producto = new Producto(
